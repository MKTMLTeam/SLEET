...
__all__ = ['AtomisticModel', 'BsAtomisticModel', 'NeuralNetworkPotential', ]
...
class AtomisticModel(nn.Module):
    ...
    def extract_outputs(
        ...
        if "attn_w" in inputs.keys():
            attn_w = inputs["attn_w"]
            return results, attn_w
        return results
...
class NeuralNetworkPotential(AtomisticModel):
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        inputs = self.postprocess(inputs)
        if "attn_w" in inputs.keys():
            results, attn_w = self.extract_outputs(inputs)
            return results, attn_w
        else:
...
class BsAtomisticModel(AtomisticModel):
    """
    ...
    def forward(self, inputs):
        ...
        # apply postprocessing (if enabled)
        inputs = self.postprocess(inputs)
        results = {inputs["property_name"]: inputs[inputs["property_name"]]}
        return results
